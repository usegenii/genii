...
/**
 * Start the Geniigotchi daemon.
 */
export function startCommand(daemon: Command): void {
	daemon
		.command('start')
		.description('Start the Geniigotchi daemon')
		...
		.action(async (options: StartOptions, cmd: Command) => {
			...
			try {
				...
				const env = {
					...process.env,
					GENIIGOTCHI_SOCKET: socketPath,
					...(options.data && { GENIIGOTCHI_DATA: options.data }),
					...(options.guidance && { GENIIGOTCHI_GUIDANCE: options.guidance }),
					...(options.pidFile && { GENIIGOTCHI_PID_FILE: options.pidFile }),
					...(options.logFile && { GENIIGOTCHI_LOG_FILE: options.logFile }),
				};
...
export function registerDaemonCommands(program: Command): void {
	const daemon = program.command('daemon').description('Manage the Geniigotchi daemon process');
