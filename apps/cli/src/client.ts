...
class SocketDaemonClient implements DaemonClient {
  ...
  private readonly _notificationHandlers: Set<
  ...
  constructor(options: DaemonClientOptions = {}) {
    ...
    this._connectTimeoutMs =
      options.connectTimeoutMs ?? DEFAULT_OPTIONS.connectTimeoutMs;
  ...
  async connect(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const connectTimeout = setTimeout(() => {
        socket.destroy();
        reject(
          new Error(`Connection timeout after ${this._connectTimeoutMs}ms`),
        );
      }, this._connectTimeoutMs);
  ...
  async listSchedulerJobs(): Promise<SchedulerJobInfo[]> {
    const result = await this._request<{ jobs: SchedulerJobInfo[] }>(
      "scheduler.list",
    );
    return result.jobs;
  }
